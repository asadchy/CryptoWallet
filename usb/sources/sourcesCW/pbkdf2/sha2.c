...
typedef BYTE  sha2_byte;	/* Exactly 1 byte */
...
void sha1_Update(SHA1_CTX* context, const sha2_byte *data, UINT32 len) {
	...
	if (usedspace > 0) {
		...
		if (len >= freespace) {
			...
			MEMCPY_BCOPY(((BYTE*)context->buffer) + usedspace, data, freespace);
			...
			MEMCPY_BCOPY(((BYTE*)context->buffer) + usedspace, data, len);
...
void sha1_Final(SHA1_CTX* context, sha2_byte digest[]) {
	...
	if (usedspace == 0) {
		...
		*context->buffer = 0x80;
		...
		/* Begin padding with a 1 bit: */
		((BYTE*)context->buffer)[usedspace++] = 0x80;

		if (usedspace <= 56) {
			/* Set-up for the last transform: */
			memzero(((BYTE*)context->buffer) + usedspace, 56 - usedspace);
		}
		else {
			if (usedspace < 64) {
				memzero(((BYTE*)context->buffer) + usedspace, 64 - usedspace);
			}
...
void sha1_Raw(const sha2_byte* data, UINT32 len, BYTE digest[SHA1_DIGEST_LENGTH]) {
	SHA1_CTX	context;
	sha1_Init(&context);
...
char* sha1_Data(const sha2_byte* data, UINT32 len, char digest[SHA1_DIGEST_STRING_LENGTH]) {
	SHA1_CTX	context;
...
void sha256_Update(SHA256_CTX* context, const sha2_byte *data, UINT32 len) {
	unsigned int	freespace, usedspace;
...
void sha256_Final(SHA256_CTX* context, sha2_byte digest[]) {
	...
	if (digest != (sha2_byte*)0) {
		...
		if (usedspace > SHA256_SHORT_BLOCK_LENGTH) {
			memzero(((BYTE*)context->buffer) + usedspace, SHA256_BLOCK_LENGTH - usedspace);
		...
		/* Set-up for the last transform: */
		memzero(((BYTE*)context->buffer) + usedspace, SHA256_SHORT_BLOCK_LENGTH - usedspace);
...
void sha256_Raw(const sha2_byte* data, UINT32 len, BYTE digest[SHA256_DIGEST_LENGTH]) {
	SHA256_CTX	context;
	sha256_Init(&context);
...
char* sha256_Data(const sha2_byte* data, UINT32 len, char digest[SHA256_DIGEST_STRING_LENGTH]) {
	SHA256_CTX	context;
	sha256_Init(&context);
	sha256_Update(&context, data, len);
...
void sha512_Update(SHA512_CTX* context, const sha2_byte *data, UINT32 len) {
	unsigned int	freespace, usedspace;
...
static void sha512_Last(SHA512_CTX* context) {
	...
	if (usedspace > SHA512_SHORT_BLOCK_LENGTH) {
		memzero(((BYTE*)context->buffer) + usedspace, SHA512_BLOCK_LENGTH - usedspace);
	...
	/* Set-up for the last transform: */
	memzero(((BYTE*)context->buffer) + usedspace, SHA512_SHORT_BLOCK_LENGTH - usedspace);
...
void sha512_Raw(const sha2_byte* data, UINT32 len, BYTE digest[SHA512_DIGEST_LENGTH]) {
	SHA512_CTX	context;
	sha512_Init(&context);
...
char* sha512_Data(const sha2_byte* data, UINT32 len, char digest[SHA512_DIGEST_STRING_LENGTH]) {
	SHA512_CTX	context;
