...
int uECC_sign_with_nonce(const uint8_t *private_key,
	const uint8_t *message_hash,
	unsigned hash_size,
	uint8_t *nonce,
	uint8_t *signature,
	uECC_Curve curve) {
	uECC_word_t k[uECC_MAX_WORDS];
	uECC_vli_bytesToNative(k, nonce, BITS_TO_WORDS(curve->num_n_bits));
	uECC_word_t tmp[uECC_MAX_WORDS];
	uECC_word_t s[uECC_MAX_WORDS];
	uECC_word_t *k2[2] = { tmp, s };
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
...
int uECC_sign_with_nonce_E(const uint8_t *private_key,
	...
	uECC_vli_nativeToBytes(signature, curve->num_bytes, p); /* store r */
	uint8_t vv[64] = {0};
	uECC_vli_nativeToBytes(vv, (curve->num_bytes)*2, p);
	*v = vv[31]%2;
#endif
